...
class FlashForge(object):
	...
	def _parse_response(self, response):
		...
		while len(response):
			start = 1
			# look for multiple commands returned in one response and parse them one by one
			nextcmd = response.find(b"CMD", start)
			# Dremel 3D40, 3D45: look for response to M119 stuck on the end of another command without CMD M119 in response
			cmdend = response.find(b"\r\nok", start)
			if cmdend > 0 and (nextcmd < 0 or cmdend < nextcmd):
				nextcmd = cmdend+4
			if nextcmd > 0:
				data = response[0:nextcmd]
				response = response[nextcmd:]
			else:
				data = response
				response = b""
			...
			if b"CMD M27 " in data:
			...
			elif b"CMD M119 " in data or b"status: ready" in data:
				...
				if len(parseddata) and not parseddata.strip().endswith(b"ok") and \
						(self._printerstate == self.STATE_READY or self._printerstate == self.STATE_SD_PAUSED):
